# GUI 架构说明

项目使用 NiceGUI 提供 Web 管理界面。GUI 负责配置管理、管道启停状态查看、模块编辑、文本输入和翻译结果展示。

```text
gui/
├── app.py                  # 初始化 GUI 状态并注册页面
├── state.py                # 全局 engine 引用和输出缓冲
├── components/
│   ├── nav.py              # 顶部导航和亮色主题约束
│   └── module_form.py      # 动态模块参数表单
└── pages/
    ├── home.py             # 首页
    ├── output_page.py      # 文本输入与输出
    ├── pipelines_page.py   # 管道管理
    ├── modules_page.py     # 模块管理
    ├── config_page.py      # 原始 JSON 编辑
    └── env_page.py         # .env 编辑
```

## 启动流程

1. `main.py` 创建 `PipelineEngine` 并加载配置。
2. `gui.create_app(engine)` 保存全局 engine 引用。
3. GUI 注册 `TerminalConsumer` 回调，用于将输出写入 `state.output_buffer`。
4. NiceGUI 启动 Web 服务；若 `.env` 的最低字段全部为空，本次程序运行第一次进入首页时自动跳转到 `/env`。该引导只触发一次，之后即使尚未填写也可以返回首页。
5. 后台更新检查完成后将结果写入共享状态，首页定时读取但不阻塞页面打开。
6. 后台线程构建并启动管道，不阻塞页面打开。

## 全局状态

`gui/state.py` 维护：

| 名称 | 说明 |
|------|------|
| `_engine` | 全局 `PipelineEngine` |
| `output_buffer` | 最近翻译输出，最多 200 条 |
| `output_lock` | 输出缓冲线程锁 |
| `_engine_init_status` | `initializing` / `ready` / `error` |
| `_engine_init_error` | 初始化错误信息 |
| `_update_info` | 后台检查得到的线上最新版本、更新说明和是否需要升级 |
| `_application_shutdown_requested` | GUI 请求整个程序安全退出的线程事件，由主线程读取 |

输出记录结构：

```python
{
    "pipeline": "管道名称",
    "original": "原文",
    "translated": "译文",
}
```

## 导航栏

所有页面顶部都调用 `create_nav()`。

| 页面 | 路由 |
|------|------|
| 首页 | `/` |
| 文字输入与输出 | `/output` |
| 管道管理 | `/pipelines` |
| 模块目录 | `/modules` |
| 配置编辑 | `/config` |
| 环境变量 | `/env` |

界面固定使用亮色主题，导航栏不提供深色模式开关。`create_nav()` 会在每个
页面创建时显式禁用 NiceGUI 深色模式；旧配置中即使仍有 `gui.dark_mode`
字段也不再读取或写入，不会影响界面主题。

## 首页

路由：`/`

功能：

- 主内容区域首行显示 `version.py` 中的当前版本号。
- QQ 交流群号显示在主页正文版本号下方，不再放在顶部导航栏标题中。
- 导航栏右上角始终显示“版本说明”入口，更新检查尚未完成或网络失败时展示 `version.py` 的本地 `release_info`。
- 没有比当前版本更新的版本时，入口仍然可用；点击后展示当前版本、线上最新版本及线上 release notes。
- 发现新版本时自动弹出更新说明弹窗，“立即更新”按钮启动 exe 同级 `VRCTTP_UPDATE.exe`。
- 更新器启动成功后，更新按钮会被禁用，版本弹窗关闭，并显示“VRCTTP 将在 3 秒后自动关闭”的提示。
- 3 秒计时由一次性 NiceGUI timer 完成。计时结束后只设置 `gui.state` 的应用退出事件；`main.py` 主循环读取该事件并调用现有安全退出流程，包括 `PipelineEngine.shutdown()`，而不是由 GUI 回调直接强制终止进程。
- 更新器启动失败时不会请求关闭主程序，GUI 会保留运行并显示错误提示。
- 用户关闭新版弹窗后，右上角入口变为橙色新版本提示，并以边框脉冲动画吸引注意；点击可重新打开弹窗。
- 普通版本说明弹窗不显示“立即更新”，仅在确实检测到新版本时显示该操作。
- 显示 engine 初始化状态。
- 显示所有 pipeline 的启用状态和运行状态。
- 切换 pipeline 的 `enabled` 字段。
- 每个 pipeline 卡片可跳转到 `/pipelines?edit_pipeline=<pipeline_id>`，自动打开对应管道编辑弹窗。
- 提供“重载所有配置”操作。
- 配置中存在 `desktop_overlay` 时显示“显示桌面翻译窗口”按钮；窗口已隐藏或异常退出时恢复它，窗口已可见时不执行任何操作。
- 桌面翻译窗口采用可滚动增量历史，最多显示 30 个完整句子；新结果置顶，达到上限后自动淘汰底部最旧记录。
- 当启用的 pipeline 使用 `sync_vrc_mic=true` 时，显示 VRChat OSC 同步提醒。

注意：切换 `enabled` 只保存配置，不会立即重载运行中的管道。

## 文字输入与输出

路由：`/output`

功能：

- 扫描当前运行的 `TextInput` 实例。
- 为每个实例提供输入框。
- 输入卡片标题显示 TextInput 模块显示名称和所属管道名称。
- 用户提交文本后进入对应 `TextInput` 队列。
- 定时读取 `state.output_buffer`，显示最新翻译结果。

适合调试翻译模块，也适合在没有音频输入时手动发送文本。

## 管道管理

路由：`/pipelines`

功能：

- 查看所有 pipeline。
- 创建、编辑、复制、删除 pipeline。
- 支持通过 `/pipelines?edit_pipeline=<pipeline_id>` 直接打开指定 pipeline 的编辑弹窗；页面会在打开后清除 URL 查询参数，避免刷新重复弹窗。
- 修改 `enabled`。
- 使用显示名称编辑路由，但保存内部 `ref_id`。
- 入口模块只能选择 `PacketProducerModule` 类型模块，例如麦克风、系统环回或 GUI 文本输入。
- 路由图编辑区域以画布方式编辑 DAG 拓扑，保存时仍写回 `graph.entry` 和 `graph.routes`。

路由图编辑器行为：

| 字段 | 说明 |
|------|------|
| 入口节点 | 画布内固定存在，不能删除，只能从底部输出连接；节点内选择框只列出入口模块类型 |
| 普通节点 | 可在画布中自由拖拽定位；节点内选择框可将该节点替换为其他已配置模块实例 |
| 节点显示 | 节点下拉只显示模块 `display_name`，不拼接 `ref_id`；节点宽度会按显示名称自适应 |
| 模块选择 | 入口节点、普通节点和底部添加区的模块下拉按模块类型分组显示 |
| 连线 | 从节点下方圆点拖拽到另一个节点上方圆点，形成上进下出的连接；拖拽过程中的虚线跟随鼠标位置 |
| 连线删除 | 点击模块间连线后，连线会高亮并在中点显示红色圆形叉，点击叉删除该边 |
| 节点删除 | 普通节点右侧红色按钮会删除该节点以及所有相关边 |
| 添加 | 画布下方功能区通过模块选择框和“添加”按钮把已有模块实例加入画布 |

模块顶部输入圆点和底部输出圆点使用编辑器内的最高显示层级，并明确接收鼠标事件。已有连线的箭头经过端口位置时会显示在圆点下方，不会遮挡圆点或干扰新连线的拖拽判定。

路由图可视区域高度为 `620px`，内部画布最小高度为 `880px`，便于同时查看和编辑更多节点。管道创建/编辑弹窗也使用更宽的 `980px` 至 `1320px` 布局。

一个节点可以连接多个下游，也可以接收多个上游。点击保存管道时，GUI 会从入口节点做可达性遍历，删除未与入口相连的离散节点和边，再保存为邻接表格式。所有写操作只保存配置，不自动重载运行时。

节点位置只用于当前编辑体验，不写入 `config.json`，因此配置文件格式保持不变。

## 模块管理

路由：`/modules`

功能：

- 查看现有模块实例。
- 编辑模块 `display_name` 和参数。
- 复制模块。
- 删除模块。
- 创建新模块。
- 查看模块类型参考表。

模块编辑弹窗使用更宽的表单区域；删除确认、软件更新等其他弹窗也采用加宽布局，以减少长字段和说明文字换行。

模块实例、模块类型选择和模块类型参考表按管道流向分区展示，添加新的模块后需要在 gui/module_catalog 里注册该模块的分区：

| 分区 | 包含模块 |
|------|----------|
| 输入源 | `microphone`、`loopback`、`text_input` |
| 语音识别 | `volc_streaming_stt`、`local_stt` |
| 过滤处理 | `filter` |
| 翻译 | `volc_machine_translation`、`baidu_machine_translation`、`llm_openai_api_call` |
| 输出 | `terminal`、`osc_vrchat` |
| 其他 | 未显式归类的新模块类型 |

模块创建规则：

- 用户输入 `display_name`。
- GUI 生成稳定 `ref_id`。
- 保存 `{display_name, type, params}`。
- 后续改名只改 `display_name`，不改 `ref_id`。

## 动态模块表单

`gui/components/module_form.py` 根据模块类的 `get_config_attributes()` 自动生成控件。

| ParamType | GUI 控件 |
|-----------|----------|
| `String` / `LanguageCode` | `ui.input` |
| `Int` / `Float` | 数值输入 |
| `Bool` | `ui.switch` |
| `Password` | 密码输入框 |
| `Select` | 下拉选择 |
| `List` | JSON 数组输入 |
| `HeaderPairsB64` | 动态 header 键值对编辑器 |
| `JsonTextB64` | 多行 JSON 文本框 |

### HeaderPairsB64

用于 `llm_openai_api_call.headers_b64`。

行为：

- GUI 显示明文 header 键值对。
- 支持添加和删除行。
- 保存时序列化为 JSON object。
- 再编码为 base64 写入 `config.json`。

### JsonTextB64

用于 `llm_openai_api_call.payload_b64`。

行为：

- GUI 显示明文 JSON 文本。
- 输入时做基础 JSON 解析检查。
- 保存时将文本 base64 编码。
- JSON 格式可疑时给出 warning，但仍保留用户输入。

## 原始配置编辑

路由：`/config`

提供 `config.json` 的原始 JSON 文本编辑器。

按钮：

| 按钮 | 行为 |
|------|------|
| 保存 | JSON 校验后写回配置 |
| 保存并重载 | 保存后调用 `engine.reload_config()` |

## 环境变量编辑

路由：`/env`

读写应用目录（打包版为 exe 同级、源码版为项目根目录）的 `.env` 文件：

- 保留注释和空行。
- 敏感键使用密码输入框。
- 支持添加和删除变量。
- 程序内置的四个最低字段即使在页面中删除，也会在保存时以空值补回。
- 四个最低字段全部为空时，页面顶部显示黄色提醒框，并使用元数据中的 Markdown 中文说明展示字段作用和获取方法。
- 提醒按字段名第一个 `_` 之前的部分分组；两个 `BAIDU` 字段组合展示，提示用户它们是配套凭证。
- 每个最低字段的输入框下方常显该字段自己的 Markdown 说明，使用半透明小字展示；用户自行添加的其他字段不显示最低字段说明。
- 保存后立即同步到当前进程的 `os.environ`，并调用 `engine.reload_config()` 热重载管道，使 `${VAR}` 配置占位符无需重启应用即可生效。

删除变量时，GUI 只会移除当前进程中仍等于旧 `.env` 值的变量，避免误删外部系统环境变量。

## 输出集成

`TerminalConsumer` 调用 GUI callback 后，数据进入 `state.output_buffer`。因此 `/output` 页面展示的是终端消费者收到的结果，而不是直接监听所有模块。

如果某条管道没有连接到 `terminal`，GUI 输出页可能看不到该管道的翻译结果。
### desktop_overlay 单例限制

桌面翻译窗口是软件级单例。配置中已经存在一个 `desktop_overlay` 实例时，
“新增模块实例”的模块类型下拉列表不再显示该类型；已有实例也不显示“复制”
按钮。引擎保存和加载配置时还会做同样的后端校验，防止通过原始 JSON 创建
重复实例。删除唯一实例后，刷新模块目录页面即可再次选择该类型。
但下次软件启动时，默认模块补全逻辑会再次自动创建缺失的桌面窗口模块。
