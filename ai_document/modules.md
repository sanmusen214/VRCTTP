...
| `loopback` | `LoopbackSource` | 音频源 | 捕获系统暴露的环回或虚拟录音设备 |
...
通过 `sounddevice`/PortAudio 捕获系统暴露的环回或虚拟录音设备。Windows 是主要支持平台；如果系统没有暴露 loopback 录音设备，需要启用“立体声混音”或使用 Virtual Audio Cable/Voicemeeter 等虚拟音频设备。

主要配置：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `device_name` | `__default_system_audio__` | 环回录音设备名；`__default_system_audio__` 表示 GUI 中的“默认系统音频”，自动选择系统暴露的 loopback/虚拟录音设备 |
| `sample_rate` | `16000` | 目标采样率 |
| `vad_mode` | `2` | VAD 灵敏度 |
| `mode` | `batch` | `batch` 或 `streaming` |
| `max_segment_seconds` | `15` | 批处理模式最大语音段长度 |
...
1. 配置指定的 `device_name`。
2. 系统暴露的 loopback/虚拟录音设备中，优先匹配默认扬声器名称。
3. 任意第一个系统暴露的 loopback/虚拟录音设备。
