...
最低字段的名称和 Markdown 获取说明统一记录在 `runtime_paths.py` 的 JSON 兼容列表中。首次打开 GUI 时，如果这四个字段全部为空，程序会自动跳转到环境变量页，并显示黄色配置提醒；该强制跳转在每次程序运行中只发生一次，之后可以正常返回首页。`BAIDU_APP_ID` 和 `BAIDU_APP_KEY` 会作为同一组凭证展示。
